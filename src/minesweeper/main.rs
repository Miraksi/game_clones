...
fn tile_textures<'a> (canvas: &mut Canvas<Window>, texture_creator: &'a mut TextureCreator<WindowContext>) -> Result<(Texture<'a>, Texture<'a>), String> {
    enum TextureKind {
        Hidden,
        Revealed
    }
...
fn text_texture<'a> (texture_creator: &'a mut TextureCreator<WindowContext>, text: &str) -> Result<(Texture<'a>, Rect), String> {
...
fn number_textures<'a> (texture_creator: &'a mut TextureCreator<WindowContext>) -> Result<(Vec<Texture<'a>>, Rect), String> {
    let ttf_context = sdl2::ttf::init().map_err(|e| e.to_string())?;
