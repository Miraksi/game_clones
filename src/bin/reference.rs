...
fn dummy_texture<'a>(canvas: &mut Canvas<Window>, texture_creator: &'a mut TextureCreator<WindowContext>) -> Result<(Texture<'a>, Texture<'a>), String> {
    enum TextureColor {
        Yellow,
        White,
    }
...
fn main() -> Result<(), String> {
    ...
    let (square_texture1, square_texture2) = dummy_texture(&mut canvas, &mut texture_creator)?;
